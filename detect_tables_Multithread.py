...
def process_pdf(pdf_path, lower_threshold, output_txt_file, batch_size=5, upper_threshold=1.0):
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for batch_num in range(num_batches):
            ...
            futures.append(executor.submit(process_batch, batch_images, start_page, lower_threshold, upper_threshold))
...
def main(input_dir, lower_threshold, output_dir, batch_size=5, upper_threshold=1.0, max_workers=4):
        ...
        for root, _, files in os.walk(input_dir):
            for file in files:
                if file.lower().endswith('.pdf'):
                    ...
                    print(f"Scheduling processing for PDF: {pdf_path}")
                    futures.append(executor.submit(process_pdf, pdf_path, lower_threshold, output_txt_path, batch_size, upper_threshold))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_workers", type=int, default=4, help="Maximum number of worker threads. Default is 4.")

    args = parser.parse_args()

    main(args.input_dir, args.lower_threshold, args.output_dir, args.batch_size, args.upper_threshold, args.max_workers)
